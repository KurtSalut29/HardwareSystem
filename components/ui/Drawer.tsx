...
export default function Drawer({ open, onClose, title, children, footer, width = "sm" }: DrawerProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      />
      <div
        className={`relative w-full ${WIDTH_CLASS[width]} h-full bg-white shadow-2xl flex flex-col transition-transform duration-200 ${open ? "translate-x-0 animate-drawer-in" : "translate-x-full"}`}
      >
        {title && (
        ...
        {/* max-h + scroll so an unusually tall footer can never push its own
            controls past the bottom of the drawer with no way to reach them. */}
